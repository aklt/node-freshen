...
;(function() {
  ...
  function ServerCom(url, conf) {
    ...
    this.ws.onmessage = function(ev) {
      self.log.push(ev)
      if (conf.debug) console.log('message', JSON.stringify(ev, 0, 2))
  ...
  var ws = 'ws:' + location.hostname
  if (location.port.length > 0) ws += ':' + location.port
  window.$freshen = new ServerCom(ws /* <<CONF>> */)
})()
