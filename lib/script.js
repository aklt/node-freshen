...
;(function () {
  ...
  function documentResources () {
    ...
    var res = {}
    res.img = [].slice.call(document.getElementsByTagName('img'))
                .filter(function (el) {
                  return loc.test(el.src)
                }).map(function (el) {
                  return el.src
                })
    res.css = [].slice.call(document.styleSheets)
                .filter(function (ss) {
                  return loc.test(ss)
                }).map(function (el) {
                  return el.src
  ...
  function ServerCom (url, conf) {
    ...
    this.ws.onmessage = function (ev) {
      ...
      var json, len, reloaderFun, suffix
      try {
      ...
      if (json.change) {
        ...
        var fileNames = {}
        for (var i = 0, len = json.change.length; i < len; i++) {
