...
Server.prototype.start = function (next) {
  ...
  this.httpServer.on('listening', function () {
    var msgListen = 'Listening to ' + self.url
    if (maxTries < 20) {
      var urlObj = url.parse(self.url)
      var url2 = urlObj.protocol + '//' + urlObj.hostname + ':' + self.port
      msgListen = 'Listening to ' + url2 + ' (changed from ' + self.url + ')'
      self.url = url2
    }
    var rcws = 'node_modules/ReconnectingWebSocket/reconnecting-websocket.min.js'
    var script = [
      '..',
      '../..',
      '../../..'
